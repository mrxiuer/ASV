...
def read_ctl_file(file_path):
    ...
    with open(file_path, "r") as f:
        ...
        for line in lines[1:]:  # 跳过表头
            ...
            if len(parts) == 4:
                try:
                    index.append(float(parts[0]) if parts[0].strip() else None)
    ...
    return index, current_headings, target_headings, losses

def plot_loss(index, losses, output_path="loss_plot.png"):
    """绘制损失函数变化图像"""
    # 过滤掉 None 值
...
def plot_headings(index, current_headings, target_headings, output_path="headings_plot.png"):
    """绘制当前偏航角和目标角度对比图"""
    # 过滤掉 None 值
...
def main():
    ...
    index, current_headings, target_headings, losses = read_ctl_file(ctl_file_path)

    # 绘制损失函数变化图像
    plot_loss(index, losses, output_path="loss_plot.png")

    # 绘制当前偏航角和目标角度对比图
    plot_headings(index, current_headings, target_headings, output_path="headings_plot.png")
