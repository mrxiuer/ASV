...
class GetPosition(Node):
    def __init__(self, name):
        ...
        self.model_save_path = os.path.join(self.model_save_dir, "boat_rnn_model2.pt")
        self.optimizer_save_path = os.path.join(self.model_save_dir, "boat_rnn_optimizer2.pt")
        ...
        # 初始化RNN模型
        self.rnn_model = RNN().to(self.device)
    ...
    def controller(self):
        ...
        input_data = torch.tensor([[[self.cur_rot, angle_to_target]]], dtype=torch.float32).to(self.device)
    ...
    def train_rnn(self, input_data, angle_to_target):
        ...
        target = torch.tensor([[self.cur_rot - angle_to_target]], dtype=torch.float32,requires_grad=True,device=self.device)
    ...
    def lidar_callback(self, msg):
        ...
        # 使用贝塞尔曲线平滑路径
        self.path = self.bezier_smooth(self.path)
    ...
    def get_neighbors(self, node):
        ...
        # 合并所有邻居
        return neighbors + extended_neighbors
    ...
    def bezier_smooth(self, path, num_segments=5):
        """使用贝塞尔曲线对路径进行平滑处理"""
        if len(path) < 3:
            return path  # 路径点太少，不需要平滑
        
        smooth_path = [path[0]]  # 保留起始点
        ...
        for i in range(1, len(path)-1):
            ...
            # 计算控制点
            # 控制点1：当前点到前一个点距离的1/3处
            d1 = self.distance(p0, p1)
            t1 = min(0.3, 1.0/3.0)  # 控制平滑程度，值越小曲线越接近原始路径
            c1x = p1[0] - (p1[0] - p0[0]) * t1
            c1y = p1[1] - (p1[1] - p0[1]) * t1
            
            # 控制点2：当前点到后一个点距离的1/3处
            d2 = self.distance(p1, p2)
            t2 = min(0.3, 1.0/3.0)
            c2x = p1[0] + (p2[0] - p1[0]) * t2
            c2y = p1[1] + (p2[1] - p1[1]) * t2
            
            # 生成贝塞尔曲线点
            for t in range(1, num_segments + 1):
                t = t / (num_segments + 1)
                # 二次贝塞尔曲线: B(t) = (1-t)²·P₀ + 2(1-t)t·P₁ + t²·P₂
                x = (1-t)**2 * c1x + 2*(1-t)*t * p1[0] + t**2 * c2x
                y = (1-t)**2 * c1y + 2*(1-t)*t * p1[1] + t**2 * c2y
        ...
        # 确保平滑路径不会穿过障碍物
        self.validate_smooth_path(smooth_path, path)
        
        return smooth_path

    def validate_smooth_path(self, smooth_path, original_path):
        """验证平滑路径是否穿过障碍物，如果穿过则回退到原始路径"""
        inflated_obstacles = self.inflate_obstacles()
        
        # 检查平滑路径上的点是否在障碍物内
        for i, point in enumerate(smooth_path):
            if point in inflated_obstacles:
                # 如果点在障碍物内，找到最近的原始路径点替换它
                min_dist = float('inf')
                best_point = None
                
                for orig_point in original_path:
                    dist = self.distance(point, orig_point)
                    if dist < min_dist and orig_point not in inflated_obstacles:
                        min_dist = dist
                        best_point = orig_point
                
                if best_point:
                    smooth_path[i] = best_point
                else:
                    # 如果找不到安全点，使用find_nearest_safe_point
                    smooth_path[i] = self.find_nearest_safe_point(point, inflated_obstacles)
